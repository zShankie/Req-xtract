...
@st.cache_resource
def load_model():
    model_path = "fine_tuned_distilbert"  # Ensure this directory exists with your fine-tuned model
...
class RequirementsExtractor:
    ...
    def identify_requirements(self, text):
        ...
        for i, sentence in enumerate(sentences):
            ...
            if is_requirement:
                ...
                if any(token.lemma_ in ["shall", "must", "should", "need", "require", "provide", "support", "allow", "enable"] for token in doc):
                    # Remove extraneous ID markers (e.g., "NFR-020") from the beginning.
        ...
        # If no requirements were extracted, hallucinate some placeholders.
        if not requirements:
...
def generate_srs_document_txt(fr_list, nfr_list, project_title="My Software Project"):
    ...
    # Use default placeholders if needed
    if not fr_list:
...
def main():
    ...
    if st.session_state.requirements:
        # Separate FR and NFR lists based on the classification
